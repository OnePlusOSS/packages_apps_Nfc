...
//    private int DEFAULT_ROUTE_ID_DEFAULT = AidRoutingManager.DEFAULT_ROUTE;
    ...
    static final int MSG_FULL_RESET = 61;
    ...
    private static final String PACKAGE_OEM_AUTOTEST = "com.oem.autotest";
    /**
...
//    CardEmulationService mCardEmulationService;
  ...
  //  private RegisteredAidCache mAidCache;
    ...
    // uicc && ese route switch
    private final static String SE_SWITCH_MODE = "oneplus_nfc_security_module_key";
    ...
    public void onFullReset() {
        sendMessage(NfcService.MSG_FULL_RESET,null);
    }
        ...
        protected Void doInBackground(Integer... params) {
            ...
            switch (params[0].intValue()) {
                ...
                case TASK_BOOT:
                    ...
                    String isAging = SystemProperties.get("sys.debug.watchdog");
                    if (mPrefs.getBoolean(PREF_NFC_ON, NFC_ON_DEFAULT)) {
                        ...
                        Log.d(TAG, "NFC is off.  Do nothing");
    ...
    final class NfcAdapterService extends INfcAdapter.Stub {
        ...
        public String getDieId() throws RemoteException {
            NfcPermissions.enforceAdminPermissions(mContext);
            return getDieIdByNative();
            // return getDieIdInner();
        }
    ...
    final class NxpExtrasService extends INxpExtrasService.Stub {
        ...
        private int _open(IBinder b) {
            synchronized(NfcService.this) {
                ...
                NfcService.this.setStandbyModeOff();
        ...
        public Bundle close(String pkg, IBinder binder) throws RemoteException {
            NfcService.this.enforceNfceeAdminPerm(pkg);
            NfcService.this.setStandbyModeOn();
            Bundle result;
            try {
...
*/
            ...
            switch (msg.what) {
                ...
                case MSG_TAG_DEBOUNCE:
                 ...
                 case MSG_RESTART_WATCHDOG:
                    ...
                    break;
                case MSG_FULL_RESET:
                    Log.e(TAG,"try to recovery by reset nfc");
                    new EnableDisableTask().execute(TASK_DISABLE);
                    new EnableDisableTask().execute(TASK_ENABLE);
            ...
            synchronized(this) {
                for (PackageInfo pkg : mInstalledPackages) {
                    if (pkg != null && pkg.applicationInfo != null) {
                        if (pkg.packageName.equals(PACKAGE_SMART_CARD_SERVICE) || pkg.packageName.equals(PACKAGE_OEM_AUTOTEST)) {
    ...
    private String getCplcInner() {
        Log.d(TAG, "[get-cplc] start");
        try {
            java.lang.Process p = Runtime.getRuntime().exec("/system/bin/get-cplc-inner -c");
            BufferedReader in = new BufferedReader(new InputStreamReader(p.getInputStream()));
            String cplc = in.readLine();
            Log.d(TAG, "[get-cplc] " + cplc);
            int status = p.waitFor();
            if (status != 0) {
                Log.e(TAG, "[get-cplc] failed");
            }
            return cplc;
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private static String bytesToHexString(int startIndex,byte[] src){
    ...
    private String getDieIdByNative() {
        byte[] dieIdRaw = mDeviceHost.getDieId();
        // start from 8 to delete response header
        String dieId = bytesToHexString(8,dieIdRaw);
        Log.d(TAG,"[getDieIdByNative] " + dieId);
        return dieId;
    }

    private void setStandbyModeOff() {
        mDeviceHost.setStandbyModeOff();
    }

    private void setStandbyModeOn() {
        mDeviceHost.setStandbyModeOn();
    }

    private String getDieIdInner() {
        Log.d(TAG, "[get-die-id] start");
        try {
            java.lang.Process p = Runtime.getRuntime().exec("/system/bin/get-cplc-inner -i");
            BufferedReader in = new BufferedReader(new InputStreamReader(p.getInputStream()));
            String dieId = in.readLine();
            Log.d(TAG, "[get-die-id] " + dieId);
            int status = p.waitFor();
            if (status != 0) {
                Log.e(TAG, "[get-die-id] failed");
            }
            return dieId;
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private final class SettingsObserver extends ContentObserver {
        private final Uri SE_SWITCH_URI = Settings.System.getUriFor(SE_SWITCH_MODE);
        ...
        public void observe() {
            Log.d(TAG,"[observe] start observe setting aid route change");
            final ContentResolver resolver = mContext.getContentResolver();
            resolver.registerContentObserver(SE_SWITCH_URI, false , this);
        }

        @Override
        public void onChange(boolean selfChange, Uri uri) {
            Log.d(TAG,"[onChange] settings changed selfChange " +
                    selfChange + "uri " + uri);

            if(uri.equals(SE_SWITCH_URI)) {
                int route= Settings.System.getInt(mContext.getContentResolver()
                        ,SE_SWITCH_MODE,0);

                if (route == 0) route = 0x1;
                else if (route == 1) route = 0x2;
                else if (route == 2) route = 0x4;
