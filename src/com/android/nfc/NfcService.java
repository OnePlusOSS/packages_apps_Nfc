...
//#ifndef H2OS_EDIT
//JeffLin, 2015/05/21, Default disable NFC function.
/*
...
//    private int DEFAULT_ROUTE_ID_DEFAULT = AidRoutingManager.DEFAULT_ROUTE;
    ...
    static final int MSG_SET_CURRENT_ROUTE = 60;
    static final int MSG_NFC_RESET = 61;
    static final int MSG_SEND_UICC_BOARDCAST = 62;
  ...
  //  private RegisteredAidCache mAidCache;
    ...
    public NfcService(Application nfcApplication) {
      ...
      //  mCardEmulationService = new CardEmulationService();
        ...
        enableNfc.addAction(TelephonyIntents.ACTION_SIM_STATE_CHANGED);
        ...
        /*zuoyonghua we should use newCachedThreadPool to new thread pool
          or use AsyncTask.THREAD_POOL_EXECUTOR don't wait finish then run current , they run at same time*/
        //new EnableDisableTask().execute(TASK_BOOT);  // do blocking boot tasks
        //new EnableDisableTask().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR,TASK_BOOT);
        new EnableDisableTask().executeOnExecutor(Executors.newCachedThreadPool(),TASK_BOOT);
    }
    ...
    class EnableDisableTask extends AsyncTask<Integer, Void, Void> {
        ...
        void checkSecureElementConfuration() {
            ...
            if (Se_list != null) {
                for (int i = 0; i < Se_list.length; i++) {
                    //mDeviceHost.doDeselectSecureElement(Se_list[i]);
                    Log.d(TAG, "Select secureelement i= "+i+" secureelement  "+Se_list[i]);
                    if (Se_list[i] == UICC_ID_TYPE)
                    {
                       mSupportUICC=true;
                    }
                }
            ...
            msg.what = MSG_SEND_UICC_BOARDCAST;
            msg.obj = mSupportUICC;
    ...
    final class NfcAdapterService extends INfcAdapter.Stub {
        ...
        public boolean enable() throws RemoteException {
            ...
            new EnableDisableTask().executeOnExecutor(Executors.newCachedThreadPool(),TASK_ENABLE);
            return true;
        }
        ...
        public boolean disable(boolean saveState) throws RemoteException {
            ...
            new EnableDisableTask().executeOnExecutor(Executors.newCachedThreadPool(),TASK_DISABLE);
            return true;
        }
       ...
       @Override
        ...
        public boolean getNfcSupportUICC( ) {
          ...
          Log.d(TAG, "getNfcSupportUICC mSupportUICC :" + mSupportUICC );
          return mSupportUICC;
        }

        public boolean oemsetDefaultRoute(final int defaultRouteEntry, final int defaultProtoRouteEntry, final int defaultTechRouteEntry) {

            sendMessage(NfcService.MSG_SET_CURRENT_ROUTE, defaultRouteEntry);
            return true;
        }
        public boolean setCurrentRoute( int currentRoute )
        {
            sendMessage(NfcService.MSG_SET_CURRENT_ROUTE, currentRoute);
            return true ;
        }
        public int getCurrentRoute( )
        {
            int currentRoute;
            currentRoute=mNxpPrefs.getInt("PREF_CURRENT_ROUTE_ID",0x01);
            Log.d(TAG, "getCurrentRoute  :" + currentRoute );
            return currentRoute;
        }
    }
        ...
        private synchronized void LSReexecute()
        {
             ...
             try
             {
                 ...
                 else
                 {
                     ...
                     /*VENDOR_EDIT fix RAIN-11245  we should check string is empty ,or will cause null point crash*/
                     br.close();
                     br=null;
                     ...
                     {
                       f.delete();
                       Log.i(TAG, "ls backup file is null  delete it ");
                       return ;
                     }
                     if(this.appName.isEmpty()||
                        this.srcIn.isEmpty()  ||
                        this.respOut.isEmpty()||
                        this.status.isEmpty()
                     )
                     {
                        this.status = "true";
                        ...
                        out.close();
                        out=null;
                        Log.i(TAG, "ls backup file is null ,error , we should creat new one");
                     }
                     /*end VENDOR_EDIT*/
                 }
                 ...
                 {
                     f.delete();
                     Log.i(TAG, "IOException null point  delete it  catch "+ie);
                     return;
                 }
                 finally{
                     try{
             ...
             {
                 f.delete();
                 Log.i(TAG, "IOException null point  delete exit "+ie);
             }
         }
   ...
   // }
    final class NxpExtrasService extends INxpExtrasService.Stub {
        ...
        private int _open(IBinder b) {
           ...
           /*add synchronized for bug  RAIN-11274 */
           synchronized(NfcService.this) {
                if (mOpenEe != null) {
                ...
                /*zuoyonghua settime 30000->1000 for bug if card unstable wait 30s timeout is too long*/
...
/*            synchronized (mWaitOMACheckCert) {
    ...
    private NfcDiscoveryParameters computeDiscoveryParameters(int screenState) {
        ...
        if(mIsRoutingTableDirty) {
            ...
            int currentRoute = mNxpPrefs.getInt("PREF_CURRENT_ROUTE_ID",0x01);
            if(!mSupportUICC && currentRoute==2)
            {
                sendMessage(NfcService.MSG_SET_CURRENT_ROUTE, 1);
                Log.d(TAG, "previous support sim/ese now only supprot ese Remove sim ");
            }
            else
            {
                ...
                Log.d(TAG, "Set default Route Entry  defaultRoute=" +defaultRoute + " protoRoute= "+protoRoute+" techRoute "+techRoute);
...
*/
     ...
     */
    public int setDefaultAidRouteLoc( int defaultAidRouteEntry)
    {
        mNxpPrefsEditor = mNxpPrefs.edit();
        Log.d(TAG, "writing setDefaultAidRouteLoc index  :" + defaultAidRouteEntry);
        ...
        Log.d(TAG, "setDefaultAidRouteLoc  :" + defaultRoute );
        return defaultRoute;
    }
     ...
     * set default  Aid route entry in case application does not configure this route entry
     */
    public int setDefaultMifareDesfireRouteLoc( int DefaultMifareDesfireRoute)
    {
        mNxpPrefsEditor = mNxpPrefs.edit();
        Log.d(TAG, "setDefaultMifareDesfireRouteLoc index  :" + DefaultMifareDesfireRoute);
        mNxpPrefsEditor.putInt("PREF_MIFARE_DESFIRE_PROTO_ROUTE_ID", ((DefaultMifareDesfireRoute << ROUTE_LOC_MASK)| (mDeviceHost.getDefaultDesfirePowerState() & 0x1F)));
        mNxpPrefsEditor.commit();
        int defaultRoute=mNxpPrefs.getInt("PREF_MIFARE_DESFIRE_PROTO_ROUTE_ID",0xFF);
        Log.d(TAG, "setDefaultMifareDesfireRouteLoc  :" + defaultRoute );
        return defaultRoute;
    }
     ...
     * set default  Aid route entry in case application does not configure this route entry
     */
    public int setDefaultMifateCLTRouteLoc( int DefaultMifateCLTRoute)
    {
        mNxpPrefsEditor = mNxpPrefs.edit();
        Log.d(TAG, "setDefaultMifateCLTRouteLoc index :" + DefaultMifateCLTRoute);
        mNxpPrefsEditor.putInt("PREF_MIFARE_CLT_ROUTE_ID", ((DefaultMifateCLTRoute  << ROUTE_LOC_MASK) | (TECH_TYPE_A << TECH_TYPE_MASK))| (mDeviceHost.getDefaultMifareCLTPowerState() & 0x1F));
        mNxpPrefsEditor.commit();
        int defaultRoute=mNxpPrefs.getInt("PREF_MIFARE_CLT_ROUTE_ID",0xFF);
        Log.d(TAG, "setDefaultMifateCLTRouteLoc  :" + defaultRoute );
        return defaultRoute;
    }
    ...
    final class NfcServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (DBG) {
                 Log.d(TAG, "NfcServiceHandler message= " + msg.what );
            }
            switch (msg.what) {
                case MSG_ROUTE_AID: {
                    int route = msg.arg1;
                ...
                case MSG_APPLY_SREEN_STATE:
                    ...
                    break;
                case MSG_SET_CURRENT_ROUTE:

                    int currentRoute = (int)msg.obj;
                    mNxpPrefsEditor = mNxpPrefs.edit();
                    Log.d(TAG, "writing to preferences currentRoute  :" + currentRoute);
                    mNxpPrefsEditor.putInt("PREF_CURRENT_ROUTE_ID", currentRoute);
                    mNxpPrefsEditor.commit();

                    Log.d(TAG, "reading preferences from user  :" + mNxpPrefs.getInt("PREF_CURRENT_ROUTE_ID",0xFF));

                    int defaultRoute= setDefaultAidRouteLoc(currentRoute);
                    int protoRoute = setDefaultMifareDesfireRouteLoc(currentRoute);
                    int techRoute= setDefaultMifateCLTRouteLoc(currentRoute);
                    Log.d(TAG, "MSG_SET_CURRENT_ROUTE  currentRoute:" + currentRoute+" defaultRoute "+defaultRoute +" protoRoute "+protoRoute+" techRoute "+techRoute);
                    setDefaultRoute(defaultRoute, protoRoute, techRoute);
                    break;

                 case MSG_NFC_RESET:
                    Log.d(TAG, "MSG_NFC_RESET mState  :" + mState);
                    if (mState == NfcAdapter.STATE_ON) {
                        ...
                        try {
                          ...
                          {
                               Thread.sleep (1000);
                               Log.d(TAG, "nfcuicc disable check  i  :" + i);
                               if (mState == NfcAdapter.STATE_OFF)
                               {
                                    break;
                               }
                        ...
                        try {
                            ...
                            {
                               Thread.sleep (1000);
                               Log.d(TAG, "nfcuicc enable check  i  :" + i);
                               if (mState == NfcAdapter.STATE_ON)
                               {
                                    break;
                               }
                ...
                case MSG_SEND_UICC_BOARDCAST:

                    boolean support = (boolean)msg.obj;
                    Log.d(TAG, "uiccintent mSupportUICC "+support);
                    Intent uiccintent = new Intent();
                    uiccintent.setAction("nfc.uicc.support");
                    uiccintent.putExtra("uiccsupport", support);
                    mContext.sendBroadcast(uiccintent);
                    break;
    ...
    private final BroadcastReceiver mEnableNfc = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals( NxpConstants.ACTION_GSMA_ENABLE_NFC)) {
                ...
                Log.d(TAG, "nfcuicc "+state + " card id= "+ slotId);
                /*we only need  sim 1 (id=0) insert and  pull out*/
                if ((state.equals(IccCardConstants.INTENT_VALUE_ICC_READY ) ||
                     state.equals(IccCardConstants.INTENT_VALUE_ICC_ABSENT))&&
                    (slotId==0))
                  {
                    Log.d(TAG, "nfcuicc need rest");
                    sendMessage(NfcService.MSG_NFC_RESET, 0);
                }
            }
