...
class P2pLinkManager implements Handler.Callback, P2pEventListener.Callback {
    ...
    public void enableDisable(boolean sendEnable, boolean receiveEnable) {
        synchronized (this) {
            if (DBG) Log.d(TAG, "enableDisable sendEnable= "+ sendEnable+" receiveEnable "+receiveEnable);
                ...
                case LINK_STATE_DOWN:
                 ...
                 // Start taking a screenshot
                    ...
                    if (mSendState == SEND_STATE_PENDING) {
                        ...
                        if (mMessageToSend != null ||
                                (mUrisToSend != null && mHandoverDataParser.isHandoverSupported())) {
                                Log.i(TAG, "LLCP activated enter");
    ...
    void prepareMessageToSend(boolean generatePlayLink) {
        synchronized (P2pLinkManager.this) {
            if (DBG) Log.d(TAG, "prepareMessageToSend");
            mMessageToSend = null;
            mUrisToSend = null;
            if (!mIsSendEnabled) {
                return;
        ...
        synchronized (this) {
            ...
            switch (mLinkState) {
                ...
                case LINK_STATE_UP:
                    ...
                    Log.i(TAG, "LINK_STATE_UP mSendState="+mSendState);
                    switch (mSendState) {
        ...
        public Void doInBackground(Void... args) {
            ...
            if (uris != null) {
                ...
                try {
                    ...
                    {
                        for (Uri uri : uris) {
                          Log.w(TAG, "doInBackground uri ."+ uri+" path= "+getRealFilePath(mContext,uri)+"  handoverResult "+handoverResult);
                        }
                    }
