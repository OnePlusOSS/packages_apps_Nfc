                ...
                case LINK_STATE_DOWN:
                 ...
                 // Start taking a screenshot
                    ...
                    if (mSendState == SEND_STATE_PENDING) {
						...
						Log.i(TAG, "rbm ======= [1] LLCP activated");//ruanbanmao add for debug
                        if (mMessageToSend != null ||
                                (mUrisToSend != null && mHandoverDataParser.isHandoverSupported())) {
                                Log.i(TAG, "rbm ======= [2] LLCP activated");//ruanbanmao add for debug
                ...
                case LINK_STATE_UP:
                    ...
                    switch (mSendState) {
                        case SEND_STATE_NOTHING_TO_SEND:
                            debounceTimeout = 0;
							Log.i(TAG, "ruan ===== SEND_STATE_NOTHING_TO_SEND!!!");//ruanbanmao add for debug
                            break;
                        case SEND_STATE_NEED_CONFIRMATION:
                            debounceTimeout = LINK_SEND_PENDING_DEBOUNCE_MS;
							Log.i(TAG, "ruan ===== LINK_SEND_PENDING_DEBOUNCE_MS!!!");//ruanbanmao add for debug
                            break;
                        case SEND_STATE_SENDING:
                            debounceTimeout = LINK_SEND_CONFIRMED_DEBOUNCE_MS;
							Log.i(TAG, "ruan ====== LINK_SEND_CONFIRMED_DEBOUNCE_MS!!!");//ruanbanmao add for debug
                            break;
                        case SEND_STATE_COMPLETE:
                            debounceTimeout = LINK_SEND_COMPLETE_DEBOUNCE_MS;
							Log.i(TAG, "ruan ===== LINK_SEND_COMPLETE_DEBOUNCE_MS!!!");//ruanbanmao add for debug
                            break;
                        case SEND_STATE_CANCELED:
                            debounceTimeout = LINK_SEND_CANCELED_DEBOUNCE_MS;
							Log.i(TAG, "ruan ===== SEND_STATE_CANCELED!!!");//ruanbanmao add for debug
                    }
                    scheduleTimeoutLocked(MSG_DEBOUNCE_TIMEOUT, debounceTimeout);
					Log.i(TAG, "ruan !!!");//ruanbanmao add for debug
                    if (mSendState == SEND_STATE_SENDING) {
