...
namespace android
{
...
#if(NXP_EXTNS == TRUE)
    extern bool gIsWaitingDeactivateSleepNtf; // Temp Workaround to fix firmware bug artf164376
#endif
    ...
    extern bool setReconnectState(bool); // Temp Workaround to fix firmware bug artf164376
...
int thread_ret;
    ...
    else if(!sReaderModeEnabled && multiprotocol_flag)
    {
        ...
        multiprotocol_timer.set (500, reconfigure_poll_cb);
    }
#endif
    else
    {
...
void reconfigure_poll_cb(union sigval)
{
    ...
    clear_multiprotocol();
    multiprotocol_timer.set (500, multiprotocol_clear_flag);
}
...
//            UINT8  nfa_disable_rf_events[] = { 0x00 };    /*commented to eliminate unused variable warning*/
    ...
    if(checkCmdSent == 1 && eventData->deactivated.type == 0)
    {
        ...
        {
#if(NXP_EXTNS == TRUE)
            if(gIsWaitingDeactivateSleepNtf)
            {
                if(eventData->deactivated.type == NFA_DEACTIVATE_TYPE_IDLE)
                {
                    eventData->deactivated.type = NFA_DEACTIVATE_TYPE_SLEEP;
...
static jboolean nfcManager_setDefaultRoute (JNIEnv*, jobject, jint defaultRouteEntry, jint defaultProtoRouteEntry, jint defaultTechRouteEntry)
{
    ...
    ALOGD ("DEFAULT PROTO ROUTE : defaultRouteEntry=%d defaultProtoRouteEntry=%d defaultTechRouteEntry=%d",defaultRouteEntry,defaultProtoRouteEntry,defaultTechRouteEntry);
#if (NXP_EXTNS == TRUE)
...
void startRfDiscovery(bool isStart)
{
    ...
    if (status == NFA_STATUS_OK)
    {
#if(NXP_EXTNS == TRUE) // Temp Workaround to fix firmware bug artf164376
        if(gIsWaitingDeactivateSleepNtf)
        {
            ALOGD ("%s: Clearing flags...", __FUNCTION__);
            if (true == getReconnectState())
                setReconnectState(false);
            gIsWaitingDeactivateSleepNtf = false;
        }
#endif
