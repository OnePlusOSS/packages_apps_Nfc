...
void SecureElement::SecElem_EeModeSet_timeout(int value)
{
     ALOGD("In SecureElement::SecElem_EeModeSet_timeout value = %d", value);
     if(value == 0x01)
     {
        SyncEventGuard guard (sSecElem.mResetEvent);
        sSecElem.mResetEvent.notifyOne();
...
static void mResetEvent_waittimeoutCallBack (union sigval)
{
    ALOGD ("%s enter", __FUNCTION__);
    SecureElement::getInstance().SecElem_EeModeSet_timeout(0x01);
}
static void mEeSetModeEvent_waittimeoutCallBack (union sigval)
{
    ALOGD ("%s enter", __FUNCTION__);
    SecureElement::getInstance().SecElem_EeModeSet_timeout(0x02);
}
...
tNFA_STATUS SecureElement::SecElem_EeModeSet(uint16_t handle, uint8_t mode)
{
    ...
    ALOGD("%s:Enter mode = %d active_ese_reset_control=%d", __FUNCTION__, mode,active_ese_reset_control);

    IntervalTimer   mResetEvent_waittimeout; // mResetEvent wait time out
    IntervalTimer   mEeSetModeEvent_waittimeout; // mResetEvent wait time out
    ...
    if((mode == NFA_EE_MD_DEACTIVATE)&&(active_ese_reset_control&(TRANS_WIRED_ONGOING|TRANS_CL_ONGOING)))
    {
        ...
        mResetEvent_waittimeout.set(7000 , mResetEvent_waittimeoutCallBack );
        ALOGD ("%s mResetEvent_waittimeout wait enter", __FUNCTION__);
        SyncEventGuard guard (sSecElem.mResetEvent);
        sSecElem.mResetEvent.wait();
        ALOGD ("%s mResetEvent_waittimeout wait exit", __FUNCTION__);
        mResetEvent_waittimeout.kill();
    ...
    {
        mEeSetModeEvent_waittimeout.set(7000 , mEeSetModeEvent_waittimeoutCallBack );
        ALOGD ("%s mEeSetModeEvent_waittimeout wait enter", __FUNCTION__);
        sSecElem.mEeSetModeEvent.wait ();
        ALOGD ("%s mEeSetModeEvent_waittimeout wait exit", __FUNCTION__);
        mEeSetModeEvent_waittimeout.kill();
    }
