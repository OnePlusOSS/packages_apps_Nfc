...
#if(NXP_EXTNS == TRUE)
    bool    gIsWaitingDeactivateSleepNtf = false; // Temp Workaround to fix firmware bug artf164376
#endif
...
static IntervalTimer sNfaVSCNotificationEventCheckTimer; // timer used for presence cmd notification timeout.
...
static void sNfaVSCNotificationEventCheckTimerProc (union sigval);
...
#if(NXP_EXTNS == TRUE)
                gIsWaitingDeactivateSleepNtf = true; // Temp Workaround to fix firmware bug artf164376
#endif
...
static jboolean nativeNfcTag_doPresenceCheck (JNIEnv*, jobject)
{
    ...
    if (NfcTag::getInstance ().mTechLibNfcTypes[handle] == NFA_PROTOCOL_ISO_DEP)
    {
        ...
        {
            {
                sNfaVSCNotificationEventCheckTimer.set(500, sNfaVSCNotificationEventCheckTimerProc);
                SyncEventGuard guard (sNfaVSCNotificationEvent);
                ...
                sNfaVSCNotificationEventCheckTimer.kill();
